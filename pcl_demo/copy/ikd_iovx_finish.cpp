...
// #include "include/laser_mapping.h"
                ...
                double &time_cost)
{
    ...
    kd_tree.setInputCloud(cl_t_st_filt);
    auto kd_size = cl_t_st_filt->size();
    ...
    for (int i = 0; i < nICP_Step; ++i)
    {
        ...
        // double kd_tree_time = measureKdTreeSearchTime(kd_tree, cloud_S_Trans, max_correspond_distance_, match_indexs);
        // total_kd_tree_time += kd_tree_time;
        // std::cout << "The kd-tree search time in iteration " << i + 1 << ": " << kd_tree_time << " seconds." << std::endl;

        // double ikd_tree_time = measureIkdTreeSearchTime(ikd_Tree, cloud_S_Trans, max_correspond_distance_, match_indexs);
        // total_ikd_tree_time += ikd_tree_time;
        // std::cout << "The ikd-tree search time in iteration " << i + 1 << ": " << ikd_tree_time << " seconds." << std::endl;

        // double ivox_time = measureIVoxSearchTime(ivox, cloud_S_Trans, max_correspond_distance_, match_indexs);
...
int main(int argc, char **argv)
{
    ...
    if (pcl::io::loadPCDFile<pcl::PointXYZ>("../materials/XPD_ship.pcd", *cloud_source) == -1)
    {
