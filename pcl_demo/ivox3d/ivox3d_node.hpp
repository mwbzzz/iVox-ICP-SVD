...
uint32_t IVoxNodePhc<PointT, dim>::CalculatePhcIndex(const PointT& pt) const {
    ...
    for (int i = 0; i < dim; ++i) {
        ...
        if (eposi(i, 0) >= std::pow(2, phc_order_)) { // 修改为 >=
            eposi(i, 0) = std::pow(2, phc_order_) - 1;
        }
    }

    // 使用 static_cast<uint8_t> 显式转换，并确保转换安全
    std::array<uint8_t, 3> apos{
        static_cast<uint8_t>(eposi(0)),
        static_cast<uint8_t>(eposi(1)),
        static_cast<uint8_t>(eposi(2))
    };

    std::array<uint8_t, 3> tmp = hilbert::v2::PositionToIndex(apos);

    uint32_t idx = (static_cast<uint32_t>(tmp[0]) << 16) + 
